...
namespace sict {
	...
	Fraction Fraction::operator * (const Fraction& rhs) const {
		...
		if (!this->isEmpty() && !rhs.isEmpty()) {
			multiNumerator = (this->numerator * rhs.numerator);
			multiDenominator = (this->denominator * rhs.denominator);
		}
		Fraction result(multiNumerator, multiDenominator);
		return result;
	}

	bool Fraction::operator == (const Fraction& rhs) const {
		bool result = false;
		if (!this->isEmpty() && !rhs.isEmpty() && this->numerator == rhs.numerator && this->denominator == rhs.denominator) {
			result = true;		
		}
		return result;
	}
