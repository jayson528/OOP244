...
namespace AMA {
	...
	std::istream& Date::read(std::istream& istr) {
		...
		int tempDay = 0;
		char garbage = '\0';
		istr >> std::setw(4) >> tempYear >> std::setw(1) >> garbage >> std::setw(2) >> tempMonth >> std::setw(1) >> garbage >> std::setw(2) >> tempDay;
		if (!istr.fail()) {
	...
	std::ostream& Date::write(std::ostream& ostr) const {
		std::cout << std::setw(2) << std::setfill('0') << year << "/" << std::setw(2) << std::setfill('0') << monthOfYear << "/" << std::setw(2) << std::setfill('0') << dayOfMonth;
		return ostr;
	}
